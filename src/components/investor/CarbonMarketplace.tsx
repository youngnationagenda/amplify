...
export function CarbonMarketplace() {
  ...
  const handleBuyCredits = async () => {
    ...
    if (buyToken === "CELO") {
      // Native CELO — no swap needed, send directly to treasury
      toast({ title: "Buy via Swap", description: "Use the Swap panel to acquire NTC carbon credits with CELO." });
      return;
    }

    // Approve + transfer to treasury (simulating marketplace buy)
    try {
      writeContract({
  ...
  const handleSellCredits = async () => {
    ...
    const ntcAmount = Number(sellAmount) * 10; // 1 credit = 10 NTC
    ...
    try {
      ...
      // Swap NTC → sellToken via Ubeswap
